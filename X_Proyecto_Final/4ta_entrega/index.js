...
// Menu seleccion unidades Economicas
document.getElementById('btnEconomicos').addEventListener('click', (e) => {
    e.preventDefault();

    valor = document.querySelector('#btnEconomicos');
    valor = valor.value;
    console.log('La seleccion fue: ' +  valor);

    if (valor == 'seleccionEconomicos'){
        for (const unidad of economicos){   
            const msg = document.querySelector('#conversorUnidades [name="msg"]');       
            msg.innerText = 'Ingresa el valor en ' + ArrayUnidades[5];
        } 
        for (const unidad of economicos){
            const menu = document.querySelector('#conversorUnidades [name="menuUnidades"]');        
            menu.innerHTML =  `
...
document.getElementById('btnCalcular').addEventListener('click', (e) => {
    ...
    (menuUnidades == 'dolar') ? res2 = calcEconomicos(inputvalue) : '';
    (menuUnidades == 'euro') ? res2 = calcEconomicos(inputvalue) : '';
    (menuUnidades == 'bitcoin') ? res2 = calcEconomicos(inputvalue) : '';
    (menuUnidades == 'uf') ? res2 = calcEconomicos(inputvalue) : '';
